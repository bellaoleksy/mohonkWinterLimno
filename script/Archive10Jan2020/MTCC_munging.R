...
AnnualData<-as.data.frame(matrix(ncol=12,nrow=length(as.numeric(levels(factor(DailyInterpol$year))))))
names(AnnualData)<-c("Year","MaxStability_Jperm2","DayMaxStability","AUC.Stability_Jperm2day","StartOfStratification_Day","EndOfStratification_Day","LengthOfStrat_days","Temp.Epi.summer","Temp.Hypo.summer","ThermoclineSlope_mperd","Temp.Epi.strat","Temp.Hypo.strat")
...
#This is the composite median stability breakpoint analysis (day 117, or April 27, the median composite stability is 64.9)
Stability.cutoff<-64.9
...
#Calculate the average from the surface (1-3m) and hypolimnion (10-12m) for that peak day
temp.Annualmeans<-as.data.frame(matrix(ncol=2,nrow=sum(!is.na(AnnualData$DateMaxStability))))
names(temp.Annualmeans)<-c("Year","Temp.Epi.peak")
temp.Annualmeans$Year<-AnnualData$Year[!is.na(AnnualData$DateMaxStability)]
temp.Annualmeans$Temp.Epi.peak<-rowMeans(DailyInterpol[DailyInterpol$Date %in% AnnualData$DateMaxStability,c("Temp_1m","Temp_2m","Temp_3m")])
temp.Annualmeans$Temp.Hypo.peak<-rowMeans(DailyInterpol[DailyInterpol$Date %in% AnnualData$DateMaxStability,c("Temp_10m","Temp_11m","Temp_12m")])
...
# * TO DO: Add NAO stratified period --------


# * TO DO: Add NAO SpringMixed -----------------------

...
pdf(file=paste("figures/MohonkPreserve-AnnualHeatMaps.pdf",sep=""), width=5, height=4,family="Times")
...
#Export a bunch of heat maps in one pdf
pdf(file=paste("figures/MohonkPreserve-AnnualThermistorProfiles.pdf",sep=""), width=5, height=4,family="Times")
...
#Export a bunch of heat maps in one pdf
pdf(file=paste("figures/MohonkPreserve-AnnualThermistorProfilesDailyInterpol.pdf",sep=""), width=5, height=4,family="Times")
...
pdf(file=paste("figures/MohonkPreserve-AnalysisOfThermalStructure.pdf",sep=""), width=5, height=4,family="Times")
...
jpeg("figures/MohonkLake-YearInLife-ExploratoryFigure-MixingAction.jpg", width = 6, height = 4, units = "in",res = 300)
par(mfrow=c(1,1))
par(mar = c(0,0,0,0))
par(oma = c(4,4,2,2))
...
op <- par(no.readonly = TRUE)
jpeg("MTCC-ExploratoryFig-StartEndLengthOfStratificationFigure.jpg", width = 3.3, height = 6.3, units = "in",res = 300)
par(mfrow=c(3,1))
par(mar = c(0,0,0,0))
par(oma = c(4,4,2,2))
...
#Plot the stratified season average temperature
plot(AnnualData$StartOfStratification_Day~AnnualData$Year,xaxt='n',yaxt='n',ylim=c(65,120))
mtext(ylabels[1],side=2,line=2.2,cex=0.8)
#Y axis label
axis(side=2,at=seq(70,110,by=10))

#Plot the stratified season average temperature
plot(AnnualData$EndOfStratification_Day~AnnualData$Year,xaxt='n',yaxt='n',ylim=c(300,345))
mtext(ylabels[2],side=2,line=2.2,cex=0.8)
#Y axis label
axis(side=2,at=seq(300,340,by=10))

#Plot the stratified season average temperature
plot(AnnualData$LengthOfStrat_days~AnnualData$Year,xaxt='n',ylim=c(190,270))
mtext(ylabels[3],side=2,line=2.2,cex=0.8)
#X axis labels    
mtext("Year",side=1,line=2.2,cex=0.8)
axis(1,at=seq(200,260,20))
axis(1,at=seq(1985,2015,10),labels=seq(1985,2015,10))
...
##Mann Kendall rank correlation for p-value from Kendall package
##Sen slope and intercept from zyp pack
#StartofStratification
y<-AnnualData$StartOfStratification_Day
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"StartOfStratification_Day"]) 
#End of stratification
y<-AnnualData$EndOfStratification_Day
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"EndOfStratification_Day"]) 
#Length of stratification
y<-AnnualData$LengthOfStrat_days
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"LengthOfStrat_days"])
#Temp.Epi.summer
y<-AnnualData$Temp.Epi.summer
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"Temp.Epi.summer"])
####*Calculate residuals from sens slope####
resids<-y-(zyp.sen(y~year)$coefficients[1]+year*zyp.sen(y~year)$coefficients[2]) 
plot(y~year)
abline(zyp.sen(y~year)$coefficients)
plot(resids~year)
abline(h=0)

#Temp.Hypo.summer
y<-AnnualData$Temp.Hypo.summer
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"Temp.Hypo.summer"]) 
#AUC.Stability_Jperm2day
y<-AnnualData$AUC.Stability_Jperm2day
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"AUC.Stability_Jperm2day"]) 
#MixingAction_gigaJday
y<-AnnualData$MixingAction_gigaJday
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"MixingAction_gigaJday"]) 
#MaxStability_Jperm2
y<-AnnualData$MaxStability_Jperm2
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"MaxStability_Jperm2"])
#Temp.Epi.strat
y<-AnnualData$Temp.Epi.strat
zyp.sen(y~year)
Kendall(y,year)
#Temp.Hypo.strat
y<-AnnualData$Temp.Hypo.strat
zyp.sen(y~year)
Kendall(y,year)
#Temp.Epi.peak
y<-AnnualData$Temp.Epi.peak
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"Temp.Epi.peak"])
#Temp.Hypo.peak
y<-AnnualData$Temp.Hypo.peak
zyp.sen(y~year)
Kendall(y,year)
pettitt.test(AnnualData[,"Temp.Hypo.peak"])


#Need to impute missing years - 1997 and 2014 with average from surrounding 2 years for each
#Extend data out to include missing years
AnnualData.impute<-merge(AnnualData,data.frame(Year=seq(min(AnnualData$Year),max(AnnualData$Year),by=1)),by="Year",all.y=T)
#Impute data for 1997
AnnualData.impute[AnnualData.impute$Year==1997,!(names(AnnualData.impute)=="Year"|names(AnnualData.impute)=="DateMaxStability")]<-colMeans(AnnualData.impute[AnnualData.impute$Year==1995|AnnualData.impute$Year==1996|AnnualData.impute$Year==1998|AnnualData.impute$Year==1999,!(names(AnnualData.impute)=="Year"|names(AnnualData.impute)=="DateMaxStability")])
#Impute data for 2014
AnnualData.impute[AnnualData.impute$Year==2014,!(names(AnnualData.impute)=="Year"|names(AnnualData.impute)=="DateMaxStability")]<-colMeans(AnnualData.impute[AnnualData.impute$Year==2012|AnnualData.impute$Year==2013|AnnualData.impute$Year==2015|AnnualData.impute$Year==2016,!(names(AnnualData.impute)=="Year"|names(AnnualData.impute)=="DateMaxStability")])
####*SENS SLOPE for the three variables of interest, imputed to use sens.slope in trend package####
sens.slope(AnnualData.impute[,"StartOfStratification_Day"])
sens.slope(AnnualData.impute[,"EndOfStratification_Day"])
sens.slope(AnnualData.impute[,"LengthOfStrat_days"])

sens.slope(AnnualData.impute[,"Temp.Epi.summer"])
sens.slope(AnnualData.impute[,"Temp.Hypo.summer"])
sens.slope(AnnualData.impute[,"AUC.Stability_Jperm2day"])
sens.slope(AnnualData.impute[,"MixingAction_gigaJday"])

sens.slope(AnnualData.impute[,"MaxStability_Jperm2"])
sens.slope(AnnualData.impute[,"Temp.Epi.strat"])
sens.slope(AnnualData.impute[,"Temp.Hypo.strat"])
sens.slope(AnnualData.impute[,"Temp.Epi.peak"])
sens.slope(AnnualData.impute[,"Temp.Hypo.peak"])

#This gives the breakpoint for which values change
pettitt.test(AnnualData[,"LengthOfStrat_days"])



#STATS: summer epi temps -----------------------------------------


# #Create a data frame for export
# Epi.summer.temperature<-AnnualData[,c("Year","Temp.Epi.summer")]
# #Change data frame name to include units
# names(Epi.summer.temperature)[2]<-"SurfaceLakeTemperatureInSummer_DegreesC"
# Epi.summer.temperature$SurfaceLakeTemperatureInSummer_DegreesF<-(Epi.summer.temperature$SurfaceLakeTemperatureInSummer_DegreesC*(9/5))+32

#Export some of the data for documentaries
# write.csv(Epi.summer.temperature,"data/SurfaceLakeTemperatureInSummer.csv",row.names = F)
...
pettitt.test(Stability.composite$Median.stability_Jperm2)
  Stability.composite$Median.stability_Jperm2[117]
...
min(AnnualData$Temp.Epi.summer)

#Plot the stratified season average temperature
plot(AnnualData$Temp.Epi.summer~AnnualData$Year,
...
#Plot the stratified season average temperature
plot(AnnualData$Temp.Hypo.summer~AnnualData$Year,
     xaxt='n',yaxt='n')

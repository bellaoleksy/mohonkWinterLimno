...
variability_sequential<-do.call(bind_rows,datalist_sequential) 
  ...
  for(model.index in 1:length(variability_sequential_fits_singleSegment$segment_length)){
    year = seq(min(MohonkIce$Year), max(MohonkIce$Year))
    datalist_sensfits[[model.index]] <- tibble(
      year = year,
      duration_sensSlope_fit_interpolate =
        variability_sequential_fits_singleSegment$duration_sensSlope_slope[model.index] *
        year + variability_sequential_fits_singleSegment$duration_sensSlope_intercept[model.index],
      iceIn_sensSlope_fit_interpolate =
        variability_sequential_fits_singleSegment$iceIn_sensSlope_slope[model.index] *
        year + variability_sequential_fits_singleSegment$iceIn_sensSlope_intercept[model.index],
      iceOut_sensSlope_fit_interpolate =
        variability_sequential_fits_singleSegment$iceOut_sensSlope_slope[model.index] *
        year + variability_sequential_fits_singleSegment$iceOut_sensSlope_intercept[model.index],
